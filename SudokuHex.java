...
/**
 * @class SudokuHex
 * @brief Representaci�n del sudoku que se quiere resolver
 */
public class SudokuHex {
    ...
    Aquí si se saca se usa,si se saca pa enseñarla eres un parguela
    /**
     * @fn SudokuHex
	...
	int i=0;
        //pasamos a min�sculas, para aceptar tambi�n sudokus escrito en mayus
        ...
        //Con los valores de las casilla, realizamos la primera actualizaci�n
        //Cada una de las casilla vacias tendr�n todos los valores posibles
    ...
    /**
     * @fn solve
     * @brief funci�n principal de la pr�tica. Se encarga de explorar el 
     * �rbol de estados asociado al sudoku, primero se elige una casilla
     * no rellena y se intenta rellenar, una vez rellenada se pasa a rellenar 
     * otra nueva, si al rellenarse con un determinado n�mero el sudoku queda 
     * en esta irresoluble entonces da marcha atr�s y quita dicho n�mero,
     * probando otros n�meros.
     */
    public void solve() {
        //Elegimos la casilla m�s prometedora
        int[] pos = this.elegirCasilla();
        int fila = pos[0];
        int col = pos[1];
        //Si no hay casilla m�s prometedora es porque todas est�n rellena
        //Damos el sudoku por resuleto y volvemos atr�s
        if(fila<0 || col<0){
            this.fin = true;
            return;
        }
        ...
        for (int i=0; i<16; i++){
            ...
            if (this.matriz[fila][col].posibles[i]==true){
                ...
                //volvemos a llamar la funci�n solve
                this.solve();
                //Si el sudoku ya esta resuelto y quedan n�meros que probar
                //cortamos dichas pruebas, ya no nos hacen falta
                if(this.fin){return;}
            }
        }
        //Si ninguno de los n�mero probados nos ha funcionado,
        //dejamos al casilla sin rellenar y la vuelta atr�s nos har� probar
        //otro n�mero en una casilla anteriormente rellenada
    ...
    /**
     * @fn elegirCasilla
     * @brief Elige la casilla m�s prometedora a ser rellenada.
     * Dicha casilla ser� aquella que est� vac�a y tiene el menor n�mero de
     * valores posibles.
     * @return array con la fila y columna de la casilla m�s prometedora
     */
    public int[] elegirCasilla(){
        ...
        for (int fila=0; fila<16; fila++){
            for (int col=0; col<16; col++){
                //Si la casilla est� vacia la tenemos en cuenta
                if (this.matriz[fila][col].valor == '.'){
                    tam = this.matriz[fila][col].numPosibles();
                    //si su n�mero de posibles es le menor de hasta ahora
    ...
    /**
     * @fn compruebaFila
     * @brief comprueba si un valor est� ya o no en una fila
     * @param fila fila a la que se le hace la comprobaci�n
     * @param valor valor el cual se quiere ver si est� en la fila
     * @return true si el valor a�n no est� en la fila
     */
    public boolean compruebaFila(int fila, char valor){
        ...
        for (int col=0; col<16; col++){
            ...
            //que aparezca m�s veces en dicha fila
   ...
   /**
     * @fn compruebaCol
     * @brief comprueba si un valor est� ya o no en una columna
     * @param col columna a la que se le hace la comprobaci�n
     * @param valor valor el cual se quiere ver si est� en la columna
     * @return true si el valor a�n no est� en la columna
     */
    public boolean compruebaCol(int col, char valor){
        //iteramos sobre la fila
        for (int fila=0; fila<16; fila++){
            //si el valor esta en alguna casilla del la columna ya no es posible
            //que aparezca m�s veces en dicha columna
            if (this.matriz[fila][col].valor == valor){
                return false;
            }
        }
     ...
     * @param valor valor que se quiere comprobar si est� o no en el sector
     * @return true si el valor a�n no est� en el sector
     */
    public boolean compruebaSector(int fila, int col, char valor){
        //Hayamos los l�mites del sector de la casilla
        int filaInicioSector = this.InicioSector(fila);
        int colInicioSector = this.InicioSector(col);
        int filaFinSector = filaInicioSector+4;
        int colFinSector = colInicioSector+4;
        ...
        for (int i=filaInicioSector; i<filaFinSector; i++){
            for(int j=colInicioSector; j<colFinSector; j++){
                //Si en algu�n momento encontramos el valor,
                //ya sabemos que valor ya est� en el sector
     ...
     * las restricciones que dicho valor impon�a al resto de casilla
     * @param fila fila de la casilla que se ha vuelto a poner vacia
     * @param col columna de la casilla que se ha vuelto a poner vac�a
     */
